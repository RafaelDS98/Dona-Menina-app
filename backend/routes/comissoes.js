      ...
      for (const colab of colabs.rows) {
        const itens = await pool.query(`
          SELECT aic.valor_comissao, aic.percentual_comissao as percentual_participacao,
            aic.comissao_padrao_usada,
            ai.preco_cobrado, ai.descricao as servico,
            DATE(a.data_hora) as data, c.nome as cliente
        ...
        const comissoesPadrao = itens.rows.map(i => parseFloat(i.comissao_padrao_usada || 0)).filter(v => v > 0);
        const percentualMedio = comissoesPadrao.length
          ? Math.round(comissoesPadrao.reduce((s, v) => s + v, 0) / comissoesPadrao.length)
          : (colab.comissao_padrao || 0);
